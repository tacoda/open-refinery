...
def fixture(oversight="assisted", **process_kw):
    ...
    return conn, dev, platform, admin, item
...
def test_chained_platform_then_admin():
    conn, dev, platform, admin, item = fixture(approval_chain=["platform", "admin"])
    audit = SqliteSink(conn)
    req = request_approval(conn, item.id, "done", dev.id, audit)
    ...
    r1 = approve(conn, req.id, platform.id, audit)   # slot 0: platform
    assert r1.status == "pending" and len(r1.approvals) == 1
    assert get_work_item(conn, item.id).current_stage == "todo"  # not applied yet

    r2 = approve(conn, req.id, admin.id, audit)  # slot 1: admin
    assert r2.status == "applied"
    assert get_work_item(conn, item.id).current_stage == "done"


def test_same_user_cannot_sign_twice():
    conn, dev, platform, admin, item = fixture(approval_chain=["platform", "platform"])
    audit = SqliteSink(conn)
    req = request_approval(conn, item.id, "done", dev.id, audit)
    approve(conn, req.id, platform.id, audit)          # slot 0
    with pytest.raises(PolicyDenied):
        approve(conn, req.id, platform.id, audit)      # same user, slot 1 → rejected


def test_reject_stops_the_request():
    conn, dev, platform, admin, item = fixture()
    audit = SqliteSink(conn)
    req = request_approval(conn, item.id, "done", dev.id, audit)
    rej = reject(conn, req.id, platform.id, audit)
    assert rej.status == "rejected"
    with pytest.raises(ValueError):
        approve(conn, req.id, platform.id, audit)      # can't approve a rejected request
    assert any(e.recipe == "approval-rejected" for e in query_events(conn))


def test_request_needs_a_gated_move():
    conn, dev, platform, admin, item = fixture(oversight="dark")  # nothing needs approval
    with pytest.raises(ValueError):
