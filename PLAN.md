...
| 1.4.0 ✅ | **Ingest polish** — GitLab reader (parity with GitHub); per-repo integration linking (`Repository.integration_id`, source-picker in Repos, host fallback); richer code signals (CI, Dockerfile, Makefile, docs, pre-commit); readers dispatched by kind. Remaining: **scheduled ingest** (needs the post-1.0 job runner). |
