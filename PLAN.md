...
| 0.13.x  | **Target OAuth handshake** — the interactive OAuth authorize→callback→store flow for targets (parity with the GitHub integration; token then flows to the backends, which already accept it). Then: rate/concurrency windows; retention/purge & residency; cost attribution by team. |
