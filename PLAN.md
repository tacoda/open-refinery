...
| 0.13.x  | **Outbound governance depth** — rate/concurrency windows on quotas; retention/purge & residency; cost attribution by team; generic `api` target backend (still stub). |
| 0.13.x / post-1.0 | **Live UI via WebSockets** — a FastAPI WebSocket channel pushing real-time updates for streaming surfaces: CI/executor job status, live logs, long-running-task progress, and new audit events (so lists update without polling). Pairs with the background job runner (Celery note) — jobs emit progress, the socket relays it. Auth the socket with the same bearer token; fall back to polling where sockets aren't available. |
...
| 0.13.x  | **Packs bundle harness artifacts** — a pack seeds not just prose `Standard`s but the same governed artifacts a policy carries: **rules / skills / commands / agents** (today packs seed prose standards only). Artifacts are **namespaced** (company / department / team / project — exact scheme TBD). Enabling a pack seeds its artifacts (pack-tagged so disable removes them); design the pack-item ↔ Policy-artifact unification here. |
...
| 0.13.x  | **Experiment-tagged runs + control/treatment cohorts** — tag live work/executions with an `experiment_id` *and* an arm (**control** = baseline/untagged, **treatment** = the change) so runs auto-feed the before/after eval samples and stay isolated from normal metrics; tie outcomes into the debt-audit health scores. (Proper t-test/scipy for small-n too.) |
...
| 0.13.x  | **Ingest polish** — schedule ingest; per-repo integration linking (today uses the repo owner's first GitHub integration); GitLab reader; richer code-signal extraction. |
...
| 1.0.0   | Deployable release + full docs. **UI revamp** — the dashboard has grown busy/jumbled (many tabs); reorganize (group governance vs. work vs. ops; progressive disclosure per the admin-overview item). **New palette:** purple primary accent, yellow highlight, red for failures/blocking, green for successes (replaces the current blue/green/purple/orange set; keep the semantic-class approach — no raw utilities in JSX). Add **Vitest** component tests with a **mocked API** covering the different UI states (empty / populated / error / role-gated). **Schema frozen** — post-1.0 changes are additive-only (via the migration runner), no restructures. |
