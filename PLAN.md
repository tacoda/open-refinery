...
| 0.12.5  | **Config in the DB, not env** — OAuth provider client id/secret (and other settings) move to an encrypted DB settings store, editable in the UI by platform/admin. Then **only `SECRET_KEY` is required in the environment**; everything else lives in the database. |
...
| 0.16.0  | **Eval system** — run experiments and evals at each layer (project, platform, harness, charter) to test whether a change makes a **demonstrable** difference. Work can be **run through the system marked as an experiment** (tagged, isolated from normal metrics); compare runs, track results over time, tie into the health scores. |
