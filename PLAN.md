...
| **pre-2.0** | **Proactive enforcement layer (security/verification).** A gate that **stops unauthorized actions before they run** — distinct from observability/audit, which explains after the fact. Whitelist-first: an action proceeds only if explicitly permitted (default-deny), enforced at the action boundary (transitions, executor invokes, tool/command calls, target/host egress). Verify identity + intent against the policy set *before* acting; block + **record every refused attempt in the audit log** (today `enforce()` raises before acting and the denial isn't audited — close that gap). Complements the current deny-overrides policy engine (opt-in allow-by-default) with an allow-list mode; ties into roles, layers, and the governance graph. This is the shift from *legible* automation to *restrained* automation. |
| **pre-2.0** | **UI/UX revamp (round 2)** — the dashboard is still confusing/intimidating for end users. Make it **intuitive and approachable**: task-oriented flows over a wall of tabs, sensible defaults, guidance/empty-state onboarding, plain language, progressive disclosure of advanced governance. Fold in broader Vitest coverage. |
| **pre-2.0** | **Agent-run post-mortem** — a UI that assembles everything about a run (the work item / execution: transitions, invokes + failures, policy decisions & refusals, attestations, quotas, redactions, target + payload/response, timings, audit events) into one view, **deduces a likely root cause**, and **suggests follow-up actions** (e.g. add a gate, fix a contradicting rule, raise a quota, close an imitation surface, open an experiment). Reuses the audit trail + governance analysis + coverage/debt signals. |
