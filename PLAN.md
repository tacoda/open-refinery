...
| 0.13.x  | **Governance analysis — poison & drift.** Flag things that poison jobs, surfaced at each appropriate role level with **metrics + insights**: rules that **never fire** (dead), **contradictions** (mutually exclusive rules), likely **prompt injection** in artifact content, and **drift detection** (governance diverging from what's actually enforced / from a baseline). Feeds the governance landscape's currently-stubbed `violations` and the debt-audit health scores. |
| 0.13.x / 0.15.0 | **Repo-level drift & coverage.** Per repository, detect drift **within** the charter and the harness, and drift **between** charter/harness and the **code as it actually is** — three axes: charter↔harness, charter↔code, harness↔code. Report **coverage** (how much of the stated governance is actually backed) and flag **imitation surfaces**: places where the charter/harness *claim* a behavior but there is **no backing instruction and no gate** enforcing it — false coverage that reads as governed but isn't. These imitation surfaces are the prime action targets (highest risk, cheapest to close). Feeds per-repo health scores + insights (with the 0.15.0 debt audits). |
