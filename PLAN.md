...
| 0.13.x  | **Remaining target backends** — a Claude-independent **OpenAI** backend + the **MCP** transport (both register in `MODEL_BACKENDS`/executors, stub for now); targets token-**or-OAuth** connect (parity with integrations); rate/concurrency windows; retention/purge & residency; cost attribution by team. |
