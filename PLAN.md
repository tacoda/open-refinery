...
| **pre-2.0** | **Proactive enforcement layer (security/verification).** A gate that **stops unauthorized actions before they run** — distinct from observability/audit, which explains after the fact. Whitelist-first: an action proceeds only if explicitly permitted (default-deny), enforced at the action boundary (transitions, executor invokes, tool/command calls, target/host egress). Verify identity + intent against the policy set *before* acting; block + record the denial. Complements the current deny-overrides policy engine (which is opt-in allow-by-default) with an allow-list mode; ties into roles, layers, and the governance graph. This is the shift from *legible* automation to *restrained* automation. |
