...
def landscape(session: Session) -> dict:
    ...
    key = lambda p: (rank_of[p.id], layer_rank(p.layer))  # lattice: role rank, then artifact layer

    def brief(p: Policy) -> dict:
        return {"id": p.id, "effect": p.effect, "role": p.role, "action": p.action,
                "resource": p.resource, "strict": p.strict, "layer": p.layer,
                "author_role": author_role.get(p.owner_id, ""), "rank": rank_of[p.id]}

    # group rules by role-rank layer, highest first
    layers: dict[int, list] = {}
    for p in rules:
        layers.setdefault(rank_of[p.id], []).append(brief(p))
    layered = [{"rank": r, "rules": layers[r]} for r in sorted(layers, reverse=True)]

    # what overrides what: a strict rule shadowing a lower-lattice, opposite-effect rule
    overrides = []
    for win in rules:
        if not win.strict:
            continue
